...
@socketio.on('joined', namespace='/startnightform')
def joined(message):
...
@socketio.on('check', namespace='/startnightform')
def uncheck(message):
...
@socketio.on('uncheck', namespace='/startnightform')
def check(message):
    """Sent by a client when the user checks a box.
    The box is checked for all people in the room."""
    room = session.get('room')
...
@socketio.on('updatebar', namespace='/startnightform')
...
@socketio.on('left', namespace='/startnightform')
def left(message):
